...
class CameraPet(CameraBase):
    ...
    def prepare_for_detection(self, display=False):
        ...
        Returns:
            True if background was successfully initialized
        """
        logging.info("Preparing for PET bottle detection")

        # Initialize the camera 
        if not self.camera_ready:
            self.init_camera()

        # Create background subtractor with reduced history parameter
        # History parameter is equal to the number of rames used to model the background, kumbaga ito 'yong tiniginan ng subtractor bago siya mag subtract 
        # Like history = 100 -> past 100 frames bago siya mag subtract
        # Lower history = Quick adaption to changes -> Suitable for changing -> In placing bottle
        ...
        # Learn initial background without bottle
        learn_frames = 30 # FPS frame

        for _ in range(learn_frames):
            ret, frame = self.camera.read()
            if not ret:
                logging.error("Failed to capture frame duing background learning")
                return False

            # apply to backgroudn model
            self.fgbg.apply(frame)

            # For checking only the display
            if display:
                progress = int((_ / learn_frames) * 100)
                ...
                cv2.imshow("Background Learning", frame_copy)
                key = cv2.waitKey(1) & 0xFF
                if key == ord('q'):
                    break

        logging.info("Background model initializedd ready for bottle placement")

        if display:
            # Clear the window 
            blank = np.zeros_like(frame)
            cv2.putText(blank, "Background learned successfully!", (30, 30), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.putText(blank, "Place bottle in green box now", (30, 60), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.imshow("Background Learning", blank)
            cv2.waitKey(1500)  # Show message for 1.5 seconds
            cv2.destroyWindow("Background Learning")
    ...
    def infer(self, timeout=10.0, display=True):
        ...
        # If background subtractor not initialized yet
        if self.fgbg is None:
            logging.warning("Background subtractor not initialized, initialized now")
            self.prepare_for_detection(display)
